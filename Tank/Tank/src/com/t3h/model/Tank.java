...
public abstract class Tank { //class cha
    ...
    public Tank(int x, int y, int orient) { //phương thức khởi tạo
        this.x = x;
        this.y = y;
        this.orient = orient;
    }
    ...
    public void move(ArrayList<Map> arr) { //ko di chuyển ra khỏi frame
        int xR = x;
        int yR = y;
        switch (orient) {
            case LEFT:
    ...
    public void fire(ArrayList<Bullet> arr) {//bắn
        long T = System.currentTimeMillis();//time bắn/ T: time hiện tại
        if (T - t < 500) {
            return;
        }
    ...
    public Rectangle getRect() { //va chạm"tạo hình chữ nhật"
    ...
    public boolean checkDie(ArrayList<Bullet> arr){ // kiểm tra va chạm
        for(int i=0;i<arr.size();i++){

            Rectangle rect = getRect().intersection(arr.get(i).getRect());
            if(rect.isEmpty()==false){//giống"ko va chạm" > xóa đạn
                arr.remove(i);
